...
class TestBattleObservations(TestGameObserver):
    def setUp(self):
        ...
        self.assertEqual(self.game_observer.action_count, 9) # 5 battles and 4 transfers
    ...
    def run_battle_helper(self, attacker_territory_id: int, defender_territory_id: int):
        ...
        self.game_observer.on_action_taken(battle_action, previous_state, self.game_state)
        ...
        if self.game_state.current_territory_transfer == (attacker_territory_id, defender_territory_id):
            ...
            self.game_observer.on_action_taken(transfer_action, previous_state, self.game_state)
    
    def test_battle_win_rates(self):
        win_rates = self.game_observer.get_battle_win_rates()
        self.assertEqual(win_rates[0], 4/5)
        self.assertEqual(win_rates[1], 0.0)
    
    def test_average_battles_per_turn(self):
        average_battles_per_turn = self.game_observer.get_average_battles_per_turn()
    ...
    def test_territory_battle_counts(self):
        territory_battle_counts = self.game_observer.get_territory_battle_counts()
