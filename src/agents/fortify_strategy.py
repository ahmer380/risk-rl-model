...
class MaximumFortifyStrategy(FortifyStrategy):
    """Select the fortify action from all routes that doesn't originate from a capital territory, prioritising fortifying towards capital territories, and if no such route actions exist then select the fortify amount action that moves the most troops."""
    def __init__(self, capitals: int):
        self.capitals = capitals

    def select_action(self, valid_actions: ActionList, game_state: GameState, _risk_map: RiskMap) -> Action:
        capital_territory_ids = self.get_capital_territory_ids(game_state)
        possible_fortify_route_actions = [action for action in valid_actions.fortify_route_actions if action.from_territory_id not in capital_territory_ids]
        
        if possible_fortify_route_actions:
            return max(
                possible_fortify_route_actions,
                key=lambda action: (
                    action.to_territory_id in capital_territory_ids, # prioritise routes that fortify towards capital territories
                    game_state.territory_troops[action.from_territory_id] - game_state.territory_troops[action.to_territory_id]
                )
            )
        elif valid_actions.fortify_amount_actions:
