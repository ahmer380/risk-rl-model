...
class ObserverManager():
    ...
    def __init__(
        ...
        player_datum = [PlayerData(i) for i in range(num_players)]
        if enable_outcome_observer:
            self.observers.append(OutcomeObserver(risk_map, player_datum))
        if enable_battle_observer:
            self.observers.append(BattleObserver(risk_map, player_datum))
        if enable_temporal_observer:
            self.observers.append(TemporalObserver(risk_map, player_datum))
